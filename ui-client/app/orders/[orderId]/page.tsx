...
import {
    ...
    TableHeader,
    TableRow,
} from "@/components/ui/table"; // Keep TableCell import if used elsewhere, but it's in OrderItemsTableBody now
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Separator } from '@/components/ui/separator';
import { ArrowLeft, Package, Calendar, Clock, MapPin, Hash, User, DollarSign } from 'lucide-react';

// Import the Client Component for the table body
import OrderItemsTableBody from '@/components/OrderItemsTableBody'; // Use default import

// --- Types ---
type Order = {
    id: number; userid: number; ordertime: string; pickuptime: string; area: string;
    location: string; tax: number; tip: number; pan: string; expiryMonth: number;
    expiryYear: number; status: string;
};
...
const formatDate = (dateString: string | null | undefined) => { if (!dateString) return "N/A"; try { const date = new Date(dateString); if (isNaN(date.getTime())) return "Invalid Date"; return format(date, 'MMM d,<x_bin_440> h:mm a'); } catch (e) { return "Invalid Date"; } };
        ...
        <main className="container mx-auto px-4 py-8">
            ...
            <Card className="mb-6">
                <CardHeader>
                    ...
                    <CardDescription>Details for your order placed on {formatDate(order.ordertime)}.</CardDescription>
                </CardHeader>
                <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div> <p className="flex items-center mb-1"><Calendar className="mr-2 h-4 w-4 text-muted-foreground" /> <strong>Order Time:</strong></p> <p className="ml-6">{formatDate(order.ordertime)}</p> </div>
                    <div> <p className="flex items-center mb-1"><Clock className="mr-2 h-4 w-4 text-muted-foreground" /> <strong>Pickup Time:</strong></p> <p className="ml-6">{formatDate(order.pickuptime)}</p> </div>
            ...
            <div className="border rounded-lg overflow-hidden">
                <Table>
                    {/* Ensure no whitespace directly inside TableHeader */}
                    <TableHeader>
                        {/* Ensure no whitespace directly inside TableRow */}
                        <TableRow>
                    ...
                    {/* Ensure no whitespace directly inside TableHeader */}
                    <TableBody>
                        {/* OrderItemsTableBody handles its own whitespace */}
