...
// Define the type for items in the order cart (ensure notes is optional string)
export type OrderItem = MenuItem & {
  quantity: number;
  notes?: string; // Optional notes per item
  firstName?: string; // Optional first name per item
};
...
export function DisplayMenu({ initialItems = [], fetchError }: DisplayMenuProps) {
 ...
 const placeOrder = async (customerFirstName: string, tipAmount: number, paymentDetails: { pan: string; expiryMonth: number; expiryYear: number; }) => {
    if (orderItems.length === 0) { toast.error("Your order is empty."); return; }
    const { tax } = calculateTotal(orderItems);
    // Include payment details in the order payload
    const orderPayload = {
        ordertime: new Date().toISOString(),
        pickuptime: new Date(Date.now() + 15 * 60 * 1000).toISOString(),
        area: "Main Lobby", location: "Pickup Counter", tax: tax, tip: tipAmount, status: "pending",
        pan: paymentDetails.pan, // Add PAN
        expiryMonth: paymentDetails.expiryMonth, // Add expiry month
        expiryYear: paymentDetails.expiryYear, // Add expiry year
    };
    const apiUrl = process.env.NEXT_PUBLIC_API_URL; if (!apiUrl) { toast.error("Configuration error: API URL is missing."); return; }
    const orderApiUrl = `${apiUrl}/api/orders`;
    const loadingToastId = toast.loading("Placing your order...");
    try {
        const orderRes = await fetch(orderApiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(orderPayload) });
        if (!orderRes.ok) { throw new Error(`Failed to create order: ${orderRes.status} ${await orderRes.text() || '(No error details)'}`); }
        const createdOrder = await orderRes.json(); const orderId = createdOrder.id;
        if (!orderId) { throw new Error("Order created, but ID was not returned."); }
        // Include notes and firstName in items payload
        const itemsPayload = orderItems.map(item => ({
            itemid: item.id,
            price: item.price,
            notes: item.notes || "", // Send notes
            firstName: item.firstName || "", // Send first name assigned to item
        }));
        const itemsApiUrl = `${apiUrl}/api/items/order/${orderId}`;
        const itemsRes = await fetch(itemsApiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(itemsPayload) });
        if (!itemsRes.ok) { throw new Error(`Order #${orderId} created, but failed to add items: ${itemsRes.status} ${await itemsRes.text() || '(No error details)'}`); }
        toast.dismiss(loadingToastId); toast.success(`Order #${orderId} placed successfully!`);
        setOrderItems([]); setIsSheetOpen(false);
    } catch (error: any) { console.error("Failed to place order:", error); toast.dismiss(loadingToastId); toast.error(`Failed to place order: ${error.message}`); }
};
  ...
  return (
    <div className="w-full max-w-6xl">
      ...
      {/* Order Summary Sheet - Pass down new update functions */}
      <OrderSummary
        items={orderItems}
        isOpen={isSheetOpen}
        ...
        updateItemNotes={updateItemNotes} // Pass down notes updater
